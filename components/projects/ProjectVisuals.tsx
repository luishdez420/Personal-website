...
export function ProjectVisual({ projectKey }: { projectKey: ProjectKey }) {
  ...
  if (projectKey === "comercio-os") return <ComercioOSLandingPreview />;
  if (projectKey === "movie-room") return <CuezlyPreview />;
  if (projectKey === "macros-ai") return <NutritionPhoneMockup />;
  return <SystemStatusPanel />;
}

function ComercioOSLandingPreview() {
  const reduceMotion = useReducedMotion();

  return (
    <motion.figure
      className="relative self-center overflow-hidden rounded-3xl border border-blue-300/20 bg-[#f5f7fb] p-2 shadow-[0_24px_70px_rgba(37,99,235,0.18)] sm:p-3"
      initial={reduceMotion ? undefined : { opacity: 0.84, scale: 0.985 }}
    ...
    >
      <div className="relative overflow-hidden rounded-2xl">
        <Image
          src="/projects/comercio-os-landing.webp"
          alt="ComercioOS landing page presenting a Spanish-first commerce platform for Dominican businesses"
          width={1800}
          height={1215}
        ...
        <motion.div
          aria-hidden="true"
          className="pointer-events-none absolute inset-y-0 -left-1/3 w-1/3 skew-x-[-18deg] bg-gradient-to-r from-transparent via-white/25 to-transparent blur-sm"
